...
def predict_future_with_technical_features(best_model, input_data, start_date, end_date):
    ...
    #future_data = pd.DataFrame({'Date': pd.date_range(input_data['Date'].max() + pd.DateOffset(1), periods=num_days)})
    future_data = pd.DataFrame({'Date': pd.date_range(start_date, end_date)})
    ...
    upper_band, middle_band, lower_band = calculate_bollinger_bands(pd.concat([data_copy, future_data]))
    upper_band_values, middle_band_values, lower_band_values = upper_band.iloc[-len(future_data):].values, middle_band.iloc[-len(future_data):].values, lower_band.iloc[-len(future_data):].values
    macd_values, signal_values = calculate_macd(pd.concat([data_copy, future_data]))  # Mengubah bagian ini

    # Sesuaikan panjang data untuk memastikan konsistensi
    rsi_values = rsi_values[-len(future_data):]
    upper_band_values = upper_band_values[-len(future_data):]
    middle_band_values = middle_band_values[-len(future_data):]
    lower_band_values = lower_band_values[-len(future_data):]
    macd_values = macd_values[-len(future_data):]
    signal_values = signal_values[-len(future_data):]

    # Menambahkan fitur-fitur teknikal ke future_data
    future_data['RSI'] = rsi_values
    future_data['UpperBand'] = upper_band_values
    future_data['MiddleBand'] = middle_band_values
    future_data['LowerBand'] = lower_band_values
    future_data['MACD'] = macd_values.values
    future_data['Signal_Line'] = signal_values.values
    ...
    # Menambahkan fitur prediksi volume untuk masa depan
    #future_data['Volume_Prediction'] = predict_future_volume(input_data, len(future_data))

    # Menyesuaikan kolom agar sesuai dengan model yang ada
    # Pastikan urutan kolom sama dengan yang digunakan selama pelatihan
    future_data = future_data[['Date', 'Volume', 'MA_5', 'MA_10', 'MA_20', 'RSI', 'UpperBand', 'MiddleBand', 'LowerBand', 'MACD', 'Signal_Line', 'Year', 'Month', 'Day', 'DayOfWeek']]
    ...
    future_data['Predicted'] = future_pred
...
end_date_str = input("masukan tanggal (YYY-MM-DD): ")
...
print(future_predictions_with_technical[['Date', 'Predicted', 'Volume', 'MA_5', 'MA_10', 'MA_20', 'RSI', 'UpperBand', 'LowerBand', 'MACD', 'Signal_Line']].reset_index(drop=True))
...
plt.plot(future_predictions_with_technical['Date'], future_predictions_with_technical['Predicted'], label='Prediksi masa Depan', marker='o')  # Menambahkan garis untuk prediksi 7 hari ke depan
